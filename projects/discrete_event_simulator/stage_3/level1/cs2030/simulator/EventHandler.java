...
public class EventHandler {
   ...
   * Constructor.
   */
  public EventHandler(int seed, int numOfServers, int queueCapacity, int numOfCustomers, double arrivalRate, double serviceRate) {
    eventQueue = new PriorityQueue<>();
    serverList = new ArrayList<>();
    rng = new RandomGenerator(seed, arrivalRate, serviceRate, 0);
    stats = new Statistics();
    createServers(numOfServers, queueCapacity);
   ...
   */
  void createServers(int n, int queueCapacity) {
    for (int i = 0; i < n; i++) {
      serverList.add(new Server(i + 1, queueCapacity, rng));
    }
  }
