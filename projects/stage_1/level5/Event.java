class Event implements Comparable<Event> {
  ...
  static Event serve(Event arriveEvent, double serveTime) {
    Customer customer = arriveEvent.getCustomer();
    Customer servedCustomer = customer.setServed();

    return new Event(servedCustomer, serveTime);
  }

  static Event wait(Event arriveEvent) {
    Customer customer = arriveEvent.getCustomer();
    Customer waitingCustomer = customer.setWait();
    double arriveTime = arriveEvent.getTime();

    return new Event(waitingCustomer, arriveTime);
  }
  ...
  public String toString() {
    if (isArriveEvent()) {
      ...
      return String.format("%.3f %s waits", time, customer);
